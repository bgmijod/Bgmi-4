...
def configure_ngrok():
    ngrok_token = "2pycKcg0N4gYLfp4qIRQsst5OS3_6dhvrMzQ7S2vB8MCSYur2"
    try:
...
def update_soul_txt(public_url):
    try:
        with open("jony4.txt", "w") as file:
            file.write(public_url)
        logging.info(f"New ngrok link saved in jony4.txt")
    except Exception as e:
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("jony4.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        logging.info("Updated jony4.txt on VPS successfully.")
    except Exception as e:
        logging.error(f"Failed to update jony4.txt on VPS: {str(e)}")
